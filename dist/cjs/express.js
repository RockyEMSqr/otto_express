...
function createApp(configOrPath) {
    ...
    if (config.serveFavicon) {
        ...
        // app.use(favicon);
        app.use(favicon((0, node_path_1.join)(__dirname, '../public/favicon.ico')));
    }
    if (config.log) {
    ...
    // view engine setup
    app.set('views', (0, node_path_1.join)(config.cwd, config.views));
    if (config.viewEngine) {
        app.set('view engine', config.viewEngine);
    }
    for (let i = 0; i < config.publicFolders.length; i++) {
        app.use(express_1.default.static((0, node_path_1.join)(config.cwd, config.publicFolders[i])));
    }
