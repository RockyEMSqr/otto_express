...
function router(app, conf) {
    let cwd = process.cwd();
    let defaults = {
        controllers: (0, node_path_1.join)(cwd, '/controllers'),
        middleware: [],
        area: null
    };
    conf = { ...defaults, ...conf };
    ...
    if (dev) {
        ...
        return (req, res, next) => {
            mountDir(app, (0, node_path_1.join)(cwd, conf.controllers), conf);
            next();
        };
    }
    else {
...
function mountDir(app, dir, opts) {
    ...
    var mods = (0, utils_js_1.rrequireDir)(dir);
    for (let key in mods) {
        //module/file
        let mod = mods[key];
        for (let mkey in mod) {
...
function SetupArea(app, dir, area, ...preHanders) {
    var mods = (0, utils_js_1.requireDir)(dir);
    for (let key in mods) {
        //module/file
        let mod = mods[key];
        for (let mkey in mod) {
